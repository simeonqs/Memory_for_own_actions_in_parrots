...
# Description: Analysis of single and double repeats. 
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# Paths
path_col_id = 'ANALYSIS/CODE/colours_and_ids.R'
path_functions = 'ANALYSIS/CODE/functions'
path_data = 'ANALYSIS/DATA/parrots/Parrots_repeat.csv'

# Load functions, colours and ids
...
m_single <- ulam(
  ...
  ), data = dat_single, chains = 4, cores = 4, iter = 8000, warmup = 500)

save(m_single, file = 'RESULTS/m_single.RData')
...
save(m_double, file = 'RESULTS/m_double.RData')
